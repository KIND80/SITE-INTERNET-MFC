...
// Questionnaire (tu peux ajouter d'autres critères côté scoring)
const questions = [
...
const ComparatorPage = () => {
  ...
  const [popupCountdown, setPopupCountdown] = useState(0); // affichage UX (secs restantes)
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (siteConfig && siteConfig.whatsapp && siteConfig.whatsapp.phone) {
        WHATSAPP_PHONE = siteConfig.whatsapp.phone;
      }
    } catch {}
  }, []);

  // Annule proprement l'ancien timer si on relance une comparaison
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (topResults.length > 0) {
        ...
        // UX: afficher un mini-compte à rebours (ring + secondes)
  ...
  // Progress ring pour le petit timer de popup
      ...
      <div className="relative min-h-[calc(100vh-80px)] bg-background text-foreground overflow-hidden">
        ...
        <div className="container py-12 md:py-20 relative">
          ...
          <Dialog open={showBooking} onOpenChange={setShowBooking}>
            <DialogContent className="sm:max-w-lg relative overflow-hidden border-0 bg-white/70 dark:bg-neutral-900/60 backdrop-blur-xl shadow-2xl">
              {/* halo gradient animé */}
              ...
              <DialogHeader>
                <DialogTitle className="text-2xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-primary to-purple-500">
                  Finalisez en 1 minute avec un conseiller
                </DialogTitle>
                <DialogDescription className="text-sm">
                  Choisissez une raison et un créneau — envoi direct par
                  WhatsApp ou email.
                </DialogDescription>
              </DialogHeader>
              ...
              >
                ...
                <p className="text-[11px] text-muted-foreground">
                  Astuce : vous pouvez modifier le texte dans WhatsApp avant
                  l’envoi.
                </p>
              </motion.div>
            </DialogContent>
